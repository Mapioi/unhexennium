...
class _EquationParentState extends State<EquationParent> {
    ...
    return new Center(
      child: new Row(
        ...
        children: [
          ...
          _buildEquationInputColumn(
            EquationSide.Product, this.inputtedProducts
          )
        ]
      )
    );
  ...
  Widget _buildEquationInputColumn(EquationSide colName, List<String> alreadyInputted) {

    // Top Title
    List<Widget> textBoxes = [new Text(enumToString(colName))];
    // Already generated elements
    textBoxes.addAll(alreadyInputted.map(
        (String element) => new Text(element)
    ));
